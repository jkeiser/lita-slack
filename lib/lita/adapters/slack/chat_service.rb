  ...
  module Adapters
    class Slack < Adapter
      ...
      class ChatService
        ...
        # @param as_user [Boolean] Extra arguments to send to Slack.
        ...
        #   particular, `response["ts"]` can be used as the `ts` argument to
        #   update_message and delete_message.
        def me_message(target, text, as_user: nil)
          api.me_message(channel: channel_for(target), text: text, as_user: as_user)
        end
        ...
        def update_message(target, ts, strings=nil, **message_arguments)
          message_arguments[:text] = Array(strings).join("\n") unless strings.nil?
          api.chat_update(channel: channel_for(target), ts: ts, **message_arguments)
        end
        ...
        def delete_message(target, ts, as_user: nil)
          api.chat_delete(channel: channel_for(target), ts: ts, as_user: as_user)
        end
